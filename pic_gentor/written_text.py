...
def _sample_text(sess, args_text, translation):
    # https://github.com/Grzego/handwriting-generation/issues/3
    args_text += ' '
...
def generate(text, text_color):
    with open(os.path.join('model', 'transModel.pkl'), 'rb') as file:
    ...
    with tf.Session(config=config) as sess:
        saver = tf.train.import_meta_graph('model/model-29.meta')
        saver.restore(sess, 'model/model-29')
        images = []
